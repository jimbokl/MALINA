...
Статус: реализован первый срез Rust/SQLite + статический HTML + Rust/WASM (`F-04`). Дальнейшая архитектура должна поддерживать большой продукт без обязательной ранней сложности.
...
Сборка добавляет JSON-LD `Article` к материалам и `CollectionPage`/`ItemList` к видимым спискам каталога и журнала. Справочная карточка сорта размечается как `WebPage` с темой сорта и хлебными крошками, без `Product` и вымышленного рейтинга. Страница отзывов получает только `WebPage` и навигацию до появления опубликованных отзывов в HTML. Контракт разметки проверяется по сгенерированным страницам в `site/tests/site.test.mjs`.
...
Редакционные материалы сейчас собираются существующим Node-генератором вместе с каталогом и страницами сортов. Решение по возможному переходу на Zola — [ADR 0005](adr/0005-editorial-generator.md); технологию меняем после измеренного ограничения, а не ради обещания оценки PageSpeed. Отзывы пишет Rust/Axum API в общую SQLite; GitHub Pages предоставляет только чтение статического снимка, поэтому живой приём требует отдельного HTTPS-адреса API.
