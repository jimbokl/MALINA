...
flowchart TD
  ...
  TOOLS -. следующий инструмент .-> MULCH[Сравнение мульчи]
  PRUNING --> JOURNAL
  PRUNING --> CATALOG
  PLANTING --> PICKER
  DEPTH --> PLANTING
...
- **Инструмент → понимание → сорт:** `/instrumenty/` объединяет подбор, расчёт саженцев, проверку глубины посадки, обрезку и расчёт шпалеры. Проверка посадки ведёт к исходным руководствам, материалу о посадке и расчёту; помощник по обрезке — к материалу или каталогу; расчёт саженцев — к подбору. Следующий кандидат — сравнение мульчи по проверенным условиям применения. Неизвестные сведения не превращаются в уверенный совет.
