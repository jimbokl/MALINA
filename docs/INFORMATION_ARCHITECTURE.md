...
flowchart TD
  ...
  HOME --> TOOLS[Практические инструменты]

  CROP --> CATALOG
  CROP --> JOURNAL
  CROP --> GUIDE[Справочник и практические руководства]
  CROP --> TOOLS
  TOOLS --> PRUNING[Схема обрезки малины]
  TOOLS --> PLANTING[Расчёт саженцев]
  TOOLS --> DEPTH[Проверка глубины посадки]
  TOOLS --> TRELLIS[Расчёт шпалеры]
  TOOLS --> MULCH[Сравнение мульчи]
  PRUNING --> JOURNAL
  PRUNING --> CATALOG
  PLANTING --> PICKER
  DEPTH --> PLANTING
...
- **Инструмент → понимание → сорт:** `/instrumenty/` объединяет подбор, расчёт саженцев, проверку глубины посадки, обрезку и расчёт шпалеры. Проверка посадки ведёт к исходным руководствам, материалу о посадке и расчёту; помощник по обрезке — к материалу или каталогу; расчёт саженцев — к подбору. Сравнение мульчи различает культуру, тип плодоношения клубники и задачу, затем ведёт к первичному источнику и следующему инструменту. Календарь ухода связывает наблюдаемое событие, культуру и подтверждённый тип плодоношения с проверкой, действием, источником и следующим событием; региональные сроки пока не назначает. `/instrumenty/ekonomika-posadki/` даёт хозяйству сценарий по его числам и возвращает к расчёту посадки и подбору, не выдавая оценку урожая за прогноз. Неизвестные сведения не превращаются в уверенный совет.
...
| Спланировать число растений | `/instrumenty/raschet-sazhencev/` | Геометрический расчёт по размерам и шагам посетителя → подбор сорта; формула и ограничения показаны. |
| Понять обрезку малины | `/instrumenty/obrezka-maliny/` | Тип плодоношения и режим урожая → схема с ограничениями → материал об обрезке и каталог для проверки сорта. |
