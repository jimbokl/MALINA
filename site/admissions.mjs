...
export function admissionSection(cultivar) {
  ...
  return `<section class="section wrap admission-section" id="gosreestr"><span class="eyebrow">ОФИЦИАЛЬНЫЙ ДОПУСК</span><h2>Что есть в Госреестре</h2><ul class="admission-list">${rows}</ul><p>Допуск к использованию относится к региону реестра. Он не гарантирует зимовку, сроки созревания и урожай на отдельном участке. Для таких выводов нужны местные испытания с описанными условиями.</p></section>`;
}
