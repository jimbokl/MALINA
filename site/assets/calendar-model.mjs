export const calendarSources = Object.freeze({
  raspberry: { label: 'RHS · выращивание и обрезка малины', url: 'https://www.rhs.org.uk/fruit/raspberries/grow-your-own' },
  strawberry: { label: 'RHS · выращивание клубники', url: 'https://www.rhs.org.uk/fruit/strawberries/grow-your-own' },
...
function raspberryPhase(phase, type, scheme, frostForecast) {
  switch (phase) {
    ...
    case 'flowers': return item('Проверить цветение', 'Осмотрите цветки и побеги, отметьте фактическое начало цветения на своём участке.', 'Следите за влажностью во время длительной засухи; точную потребность проверяйте по почве.', 'raspberry', frostForecast ? 'Прогноз заморозка требует местной оценки риска; это руководство не задаёт универсальный способ защиты цветков малины.' : 'Календарная дата цветения не переносится из руководства на ваш регион.');
    ...
    case 'after':
      ...
      if (type === 'primocane') return item('Не спешить с обрезкой', 'Проверьте, полностью ли закончилось плодоношение и какой режим сбора выбран.', 'Для одного позднего урожая обрезку всех побегов планируют в период покоя, а не сразу по факту последней ягоды.', 'raspberry', scheme === 'double' ? 'При двух урожаях схема другая: проверьте её отдельно в помощнике по обрезке.' : 'Это схема одного позднего урожая.');
      return item('Сначала уточнить тип малины', 'Летняя и ремонтантная малина плодоносят на побегах разного возраста.', 'Пока тип неизвестен, не срезайте все побеги. Уточните описание сорта или наблюдайте за плодоношением.', 'raspberry', 'Без типа плодоношения схема обрезки не определена.');
    case 'dormant':
      if (type === 'primocane' && scheme === 'single') return item('Обрезка одного позднего урожая', 'Подтвердите, что это ремонтантная малина и вы выращиваете её ради одного позднего урожая.', 'В период покоя обрежьте отплодоносившие побеги по схеме одного урожая до появления новых побегов.', 'raspberry', 'Точный срок зависит от местных условий; руководство RHS описывает климат Великобритании.');
      if (type === 'primocane') return item('Сверить схему двух урожаев', 'Вы выбрали два урожая; полная обрезка уничтожит возможность раннего сбора на сохранённых побегах.', 'Откройте отдельную схему обрезки и сохраните побеги согласно выбранному режиму.', 'raspberry', 'Для двух урожаев состояние побегов и сила растения требуют отдельной проверки.');
      return item('Не назначать общую обрезку', 'Проверьте тип сорта и состояние оставленных побегов.', 'Не срезайте все побеги по схеме ремонтантной малины.', 'raspberry', type === 'unknown' ? 'Тип плодоношения пока неизвестен.' : 'Летняя малина плодоносит на побегах прошлого года.');
  }
}
...
function strawberryPhase(phase, type, frostForecast) {
  switch (phase) {
    ...
    case 'after':
      if (type === 'june') return item('Оценить уход после летнего сбора', 'Посадка действительно плодоносит однократно и будет сохранена на следующий сезон?', 'Для многолетней посадки после сбора рассмотрите удаление старых листьев, не повреждая сердечко; сначала проверьте состояние посадки и систему выращивания.', 'strawberry', 'Реновация подходит не каждой схеме; не срезайте листья автоматически.');
      ...
      return item('Уточнить тип плодоношения', 'Неизвестно, однократный это сбор или длительное плодоношение.', 'Не назначайте сплошную обрезку листьев. Уточните сорт и схему посадки.', 'strawberry', 'Без типа плодоношения послесборовый приём не подтверждён.');
    case 'dormant': return item('Оценить состояние посадки', 'Как зимуют растения и нужна ли защита именно в ваших условиях?', 'Осмотрите посадку и сверяйте защиту с местной погодой и устройством грядки.', 'systems', 'Руководства для других стран не задают дату и толщину укрытия для регионов России.');
  }
}
...
export function makeCalendar({ crop, type = 'unknown', scheme = 'single', phase, frostForecast = false }) {
  ...
  return {
    ...
    uncertainty: type === 'unknown' ? 'Тип плодоношения не подтверждён: специальные действия по обрезке не назначены.' : 'Региональные сроки не подтверждены: ориентируйтесь на наблюдаемое событие и местный прогноз.',
