...
export function evidenceSection(cultivar) {
  ...
  const cards = observations.map(item => {
    ...
    return `<article class="evidence-card"><h3>${escapeHtml(item.value_text ?? item.value_number ?? 'Свойство сорта')}</h3><p>${escapeHtml(publicEvidenceText(item.context_text))}</p><dl><div><dt>Тип основания</dt><dd>${escapeHtml(evidenceKinds[proof.evidence_kind] || 'Источник')}</dd></div><div><dt>Основание</dt><dd>${escapeHtml(publicEvidenceText(proof.subject_description))}</dd></div><div><dt>Где в источнике</dt><dd>${escapeHtml(publicEvidenceText(proof.source_locator))}</dd></div><div><dt>Метод и выборка</dt><dd>${escapeHtml(publicEvidenceText(proof.method_text || 'Не указаны в источнике'))}${proof.sample_size != null ? ` · ${escapeHtml(proof.sample_size)} наблюдений` : ''}</dd></div><div><dt>Применимость</dt><dd>${escapeHtml(publicEvidenceText(proof.applicability_note))}</dd></div><div><dt>Ограничения</dt><dd>${escapeHtml(publicEvidenceText(proof.limitations_note))}</dd></div>${checkedOn ? `<div><dt>Проверено</dt><dd><time datetime="${checkedOn}">${checkedOn.split('-').reverse().join('.')}</time></dd></div>` : ''}</dl>${url ? `<a href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(publicEvidenceText(item.source_title))} ↗</a>` : ''}</article>`;
  }).join('');
