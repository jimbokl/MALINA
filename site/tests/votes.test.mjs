...
test('рейтинг делит места при равенстве, оставляет нули без места и пересчитывает культуру', () => {
  ...
  assert.deepEqual(ranked.map(v => [v.slug, v.rank]), [['joan-j', 1], ['polka', 1], ['elan', 3], ['aziya', null], ['gusar', null], ['cambridge-favourite', null]]);
  const strawberry = rankCultivars(varieties, snapshot.votes, 'strawberry');
  assert.deepEqual(strawberry.map(v => [v.slug, v.rank]), [['elan', 1], ['aziya', null], ['cambridge-favourite', null]]);
  const empty = rankCultivars(varieties, snapshot.votes.map(v => ({ ...v, count: 0 })));
  assert.ok(empty.every(v => v.rank === null));
  assert.deepEqual(empty.map(v => v.slug), ['aziya', 'gusar', 'joan-j', 'cambridge-favourite', 'polka', 'elan']);
});
